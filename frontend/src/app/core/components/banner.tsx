...
export const Banner = ({ cell }: { cell: any }) => {
    ...
    let image = components.find((component: any) => component.id === "media")?.content?.selectedComponent?.content?.images?.[0];
    ...
    return (
        ...
        >
            <div className={`${cell.layout.colspan === 3 ? "w-2/4" : "w-5/5"} px-10 pt-10`}>
                <h1 className={`${title === "SALE" ? "text-9xl" : "text-2xl"} font-bold mb-3`}>
                    {title}
                </h1>
