    ...
    return json(
        {
            ...
            ENV: {
                ...
                SERVICE_API_URL: config.isValidPlatform()
                    ? config.getRoute('serviceapi').url.replace(/\/$/, '').replace('*', shared.config.identifier)
                    : (process.env.SERVICE_API_URL_PATTERN || '').replace('%s', shared.config.identifier),
...
const Document: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const { storeFrontConfig } = useLoaderData<{ storeFrontConfig: TStoreFrontConfig }>();
    return (
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return (
        <>
            <header className="md:container auto 2xl w-full mx-auto p-8 md:px-6">
                <script
