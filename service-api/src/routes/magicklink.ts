import { MagickLinkUserInfosPayload, magickLinkUserInfosPayload, handleMagickLinkConfirmationRequestPayload, handleMagickLinkRegisterPayload, MagickLinkRegisterArguments, MagickLinkConfirmArguments } from '@crystallize/node-service-api-request-handlers';
    ...
    "/confirm/email/magicklink/:token": {
        get: {
            ...
            args: (context: Koa.Context): MagickLinkConfirmArguments => {
                ...
                const frontendURL = config.isValidPlatform() ? config.getRoute('frontapp').url.replace(/\/$/, '').replace('*', context.superFast.identifier) : ((process.env.FRONTEND_URL_PATTERN || '').replace('%s', context.superFast.config.identifier));
                return {
